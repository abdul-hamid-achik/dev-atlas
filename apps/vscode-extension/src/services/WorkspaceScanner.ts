...
export class WorkspaceScanner {
  ...
  async scanWorkspace(options: Partial<ScanOptions> = {}): Promise<{
    ...
    try {
      ...
      log(
        `Workspace scan completed: ${results.filesScanned} files, ${results.nodesCreated} nodes, ${results.edgesCreated} edges`
      );
      return results;
    } catch (error) {
  ...
  async scanFile(fileUri: vscode.Uri): Promise<boolean> {
    try {
      ...
      const existingNode = existingNodes.find((node) => node.properties?.path === relativePath);
  ...
  async removeFile(fileUri: vscode.Uri): Promise<boolean> {
    try {
      ...
      const existingNodes = await this.db.queryNodes({ label: fileName });
      const nodeToRemove = existingNodes.find((node) => node.properties?.path === relativePath);
