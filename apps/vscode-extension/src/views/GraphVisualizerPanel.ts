...
export class GraphVisualizerPanel {
   ...
   */
  private async _handleNodeClick(
    nodeId: string,
    nodeData: { type?: string; label?: string; properties?: Record<string, unknown> }
  ) {
    try {
  ...
  private _isFileNode(nodeData: {
    ...
    if (
      label.match(
        /\.(ts|js|jsx|tsx|py|java|cpp|c|h|css|scss|html|md|json|xml|yaml|yml|config|env)$/
      )
    ) {
      return true;
    }
    ...
    if (
      label.includes('/') &&
      (label.includes('.') || label.startsWith('src/') || label.startsWith('app/'))
    ) {
      return true;
    }
   ...
   */
  private async _handleFileNodeClick(nodeData: {
    type?: string;
    label?: string;
    properties?: Record<string, unknown>;
  }) {
    ...
    if (filePaths.length === 0) {
      vscode.window.showInformationMessage(
        `File node: ${nodeData.label} (no valid file paths found)`
      );
      return;
    }
   ...
   */
  private _extractFilePaths(nodeData: {
    type?: string;
    label?: string;
    properties?: Record<string, unknown>;
  }): string[] {
    ...
    if (properties.filePath) {
      if (Array.isArray(properties.filePath)) {
        paths.push(...(properties.filePath as string[]));
      } else {
        paths.push(properties.filePath as string);
      }
    }
    ...
    if (properties.fileName) {
      paths.push(properties.fileName as string);
    }
    ...
    if (label.includes('/') || label.includes('.')) {
      ...
      if (workspaceFolders && workspaceFolders.length > 0) {
        ...
        const fullPath = require('node:path').resolve(workspaceRoot, label);
        paths.push(fullPath);
      }
  ...
  private async _showFileActionDialog(filePaths: string[]) {
    ...
    const fileOptions: vscode.QuickPickItem[] = filePaths.map((path) => ({
      label: require('node:path').basename(path),
      description: path,
    ...
    const allItems = [
      ...fileOptions,
      { kind: vscode.QuickPickItemKind.Separator, label: 'Preferences' },
      ...preferenceItems,
    ];
    ...
    if (
      selectedAction.label.includes('Open Files') ||
      selectedAction.label.includes('Open & Copy')
    ) {
      await this._openFiles(selectedFiles);
    }

    if (
      selectedAction.label.includes('Copy to Clipboard') ||
      selectedAction.label.includes('Open & Copy')
    ) {
   ...
   */
  private async _showFileOpeningDialog(filePaths: string[], currentPrefs: Record<string, unknown>) {
    ...
    if (alwaysOpenSelected) {
      ...
      vscode.window.showInformationMessage(
        'File opening preference updated: will always open files'
      );
      ...
      vscode.window.showInformationMessage(
        'File opening preference updated: will never open files'
      );
    } else {
