...
type GlobalWithProvider = typeof globalThis & {
  devAtlasKnowledgeGraphProvider?: KnowledgeGraphDB;
};
...
export async function addToKnowledgeGraphCommand(uri?: vscode.Uri) {
  ...
  try {
    ...
    const existingNode = existingNodes.find((node: Node) => node.properties?.path === relativePath);
    ...
    if (isDirectory) {
      const scanContents = await vscode.window.showQuickPick(['Yes', 'No'], {
...
export async function createRelationshipCommand(uri?: vscode.Uri) {
  ...
  try {
    ...
    const sourceNode = sourceNodes.find(
      (node: Node) => node.properties?.path === sourceRelativePath
    );

    if (!sourceNode) {
      const addSource = await vscode.window.showQuickPick(['Yes', 'No'], {
...
export async function showFileInfoCommand(uri?: vscode.Uri) {
  ...
  try {
    ...
    const node = nodes.find((n: Node) => n.properties?.path === relativePath);
...
async function getAdditionalProperties(
  ...
  if (nodeType === 'SourceFile') {
    const importance = await vscode.window.showQuickPick(['Low', 'Medium', 'High', 'Critical'], {
