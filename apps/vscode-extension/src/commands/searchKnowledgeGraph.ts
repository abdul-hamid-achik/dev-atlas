    ...
    try {
      switch (searchOptions.value) {
        case 'vector': {
          ...
          results = vectorResults.map((r: { node: Node; similarity: number }) => ({
            type: 'node' as const,
            item: r.node,
        ...
        case 'hybrid': {
          ...
          results = hybridResults.map((node: Node) => ({
            type: 'node' as const,
            item: node,
        ...
        case 'contextual': {
          ...
          const directResults = contextResults.directMatches.map((node: Node) => ({
            type: 'node' as const,
            item: node,
            score: 95,
            matchFields: ['direct'],
          ...
          const relatedResults = contextResults.relatedNodes
            .slice(0, 10)
            .map((r: { node: Node; relationship: string; distance: number }) => ({
              type: 'node' as const,
              item: r.node,
...
function calculateEdgeSearchScore(edge: Edge, searchTerm: string): number {
  ...
  if (
    edge.sourceId.toLowerCase().includes(searchTerm) ||
    edge.targetId.toLowerCase().includes(searchTerm)
  ) {
    score += 5;
  }
...
async function performAdvancedSearch(
  ...
  if (params.searchType === 'both' || params.searchType === 'nodes') {
    const nodeQuery: { limit: number; type?: string; label?: string } = { limit: 1000 };
    if (params.nodeType) {
  ...
  if (params.searchType === 'both' || params.searchType === 'edges') {
    const edgeQuery: { limit: number; type?: string } = { limit: 1000 };
    if (params.edgeType) {
 ...
 */
async function showVectorSearchResults(
  results: Array<{
    type: string;
    item: unknown;
    score: number;
  ...
  const items: SearchQuickPickItem[] = results.map((result) => {
    const item = result.item as Node | Edge;
    ...
    if (isNode && (item as Node).label && isFileLikeName((item as Node).label)) {
    ...
    const emoji =
      searchType === 'contextual' && result.category?.includes('Related')
        ? '🔗'
        : isNode
          ? '⭕'
          : '➡️';

    // Create a proper TypedSearchResult
    const typedResult: TypedSearchResult = isNode
      ? {
          type: 'node' as const,
          item: item as Node,
          score: result.score,
  ...
  const actionItems: vscode.QuickPickItem[] = [
    {
      ...
      label: `${searchType.charAt(0).toUpperCase() + searchType.slice(1)} Actions`,
    },
    {
  ...
  if (selected.label.includes('Open Graph Visualizer')) {
    ...
    vscode.window.showInformationMessage(
      'Graph visualizer opened - semantic highlighting coming soon!'
    );
    return;
  }
...
async function showSearchResults(results: TypedSearchResult[], searchTitle: string) {
  ...
  if (selected.label.includes('Open Graph Visualizer')) {
    vscode.commands.executeCommand('dev-atlas.openKnowledgeGraph');
    vscode.window.showInformationMessage(
      'Graph visualizer opened - search highlighting coming in future update'
    );
    return;
  }
...
async function handleSearchResultSelection(result: TypedSearchResult) {
  ...
  if (action.label.includes('Show Details')) {
    ...
    if (knowledgeGraphProvider?.getDatabase()) {
      const db = knowledgeGraphProvider.getDatabase();
      if (db) {
        await findConnectedItems(db, item, result.type);
      }
    }
  }
}
 ...
 */
async function findConnectedItems(db: KnowledgeGraphDB, item: Node | Edge, type: 'node' | 'edge') {
  try {
    if (type === 'node') {
      const node = item as Node;
  ...
  try {
    ...
    if (nodeResults.length > 0) {
      ...
      for (const result of nodeResults) {
        const node = result.item as Node;
    ...
    if (edgeResults.length > 0) {
      ...
      for (const result of edgeResults) {
        const edge = result.item as Edge;
 ...
 */
async function copyVectorSearchResultsToClipboard(
  results: Array<{
    type: string;
    item: unknown;
    score: number;
  ...
  try {
    ...
    if (nodeResults.length > 0) {
      ...
      for (const result of nodeResults) {
        const node = result.item as Node;
        const simScore = result.similarity
    ...
    await vscode.env.clipboard.writeText(clipboardContent);
    vscode.window.showInformationMessage(
      `Copied ${results.length} semantic search results to clipboard`
    );
  } catch (error) {
...
function isFileLikeName(name: string): boolean {
  return (
    name.includes('/') ||
    /\.(ts|js|jsx|tsx|py|java|cpp|c|h|css|scss|html|md|json|xml|yaml|yml|config|env)$/.test(name)
  );
}
